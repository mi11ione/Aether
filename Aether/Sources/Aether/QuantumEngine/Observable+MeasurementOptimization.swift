...
public extension Observable {
    ...
    /// Validate that all grouping algorithms produce correct results.
    ///
    /// - Parameters:
    ///   - state: Test quantum state
    ///   - tolerance: Acceptable error tolerance
    ///   - numQubits: Number of qubits (for unitary partitioning)
    /// - Returns: True if all methods agree within tolerance
    func validateMeasurementOptimization(
        state: QuantumState,
        ...
        if let nQubits = numQubits, nQubits <= 12 {
            ...
            if abs(exactValue - unitaryValue) > tolerance {
                print("Unitary partitioning validation failed: |exact - unitary| = \(abs(exactValue - unitaryValue))")
                return false
            }
        }
    ...
    /// Compute expectation value from QWC groups.
    private func computeExpectationFromGroups(
        groups: [QWCGroup],
        state: QuantumState
    ) -> Double {
        var total = 0.0
    ...
    /// Compute expectation value from unitary partitions.
    private func computeExpectationFromPartitions(
        partitions: [UnitaryPartition],
        state: QuantumState
    ) -> Double {
        var total = 0.0
