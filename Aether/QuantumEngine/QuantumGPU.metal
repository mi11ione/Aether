...
//  Aether
//
//  Metal compute shaders for GPU-accelerated quantum gate application
...
kernel void applySingleQubitGate(
    ...
    constant ComplexFloat *gateMatrix [[buffer(2)]],  // 4 elements: g00, g01, g10, g11
    ...
    if ((gid & bitMask) == 0 && gid < stateSize) {
        const uint i = gid;           // Index with qubit=0
        const uint j = gid | bitMask; // Index with qubit=1
...
kernel void applyTwoQubitGate(
    ...
    constant uint &controlQubit [[buffer(1)]],
    constant uint &targetQubit [[buffer(2)]],
    constant ComplexFloat *gateMatrix [[buffer(3)]],  // 16 elements (4x4)
    ...
    // Only process base indices (both qubits = 0)
