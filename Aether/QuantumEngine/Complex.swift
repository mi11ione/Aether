...
struct Complex<T: BinaryFloatingPoint>: Equatable, CustomStringConvertible, ExpressibleByIntegerLiteral, ExpressibleByFloatLiteral, AdditiveArithmetic {
    // MARK: - Properties

    nonisolated let real: T
    nonisolated let imaginary: T
...
private func cos<T: FloatingPoint>(_ value: T) -> T {
    if let d = value as? Double {
        return Foundation.cos(d) as! T
    } else if let f = value as? Float {
        return Foundation.cos(f) as! T
    }
...
private func sin<T: FloatingPoint>(_ value: T) -> T {
    if let d = value as? Double {
        return Foundation.sin(d) as! T
    } else if let f = value as? Float {
        return Foundation.sin(f) as! T
    }
...
private func atan2<T: FloatingPoint>(_ y: T, _ x: T) -> T {
    if let dy = y as? Double, let dx = x as? Double {
        return Foundation.atan2(dy, dx) as! T
    } else if let fy = y as? Float, let fx = x as? Float {
        return Foundation.atan2(fy, fx) as! T
    }
