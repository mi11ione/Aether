...
enum QuantumGate: Equatable, Hashable, CustomStringConvertible {
    ...
    var qubitsRequired: Int {
        ...
        case .identity, .pauliX, .pauliY, .pauliZ, .hadamard,
            ...
            1
        case .cnot, .controlledPhase, .swap:
            2
        case .toffoli:
            3
        }
    ...
    var isParameterized: Bool {
        switch self {
        case .phase, .rotationX, .rotationY, .rotationZ, .controlledPhase:
            true
        default:
            false
        }
    ...
    var isHermitian: Bool {
        switch self {
        case .pauliX, .pauliY, .pauliZ, .hadamard:
            true
        default:
            false
        }
    ...
    func matrix() -> [[Complex<Double>]] {
        ...
        case let .phase(theta):
            phaseMatrix(theta: theta)

        case .sGate:
            phaseMatrix(theta: .pi / 2.0)

        case .tGate:
            phaseMatrix(theta: .pi / 4.0)

        case let .rotationX(theta):
            rotationXMatrix(theta: theta)

        case let .rotationY(theta):
            rotationYMatrix(theta: theta)

        case let .rotationZ(theta):
            rotationZMatrix(theta: theta)

        case .cnot:
            cnotMatrix()

        case let .controlledPhase(theta, _, _):
            controlledPhaseMatrix(theta: theta)
    ...
    func validateQubitIndices(numQubits: Int) -> Bool {
        switch self {
        case .identity, .pauliX, .pauliY, .pauliZ, .hadamard,
             .phase, .sGate, .tGate, .rotationX, .rotationY, .rotationZ:
            // Single-qubit gates don't store indices (applied externally)
            return true

        case let .cnot(control, target):
            return control != target &&
                control >= 0 && control < numQubits &&
                target >= 0 && target < numQubits
    ...
    var description: String {
        ...
        case .hadamard:
            "H"
        case let .phase(theta):
            "P(\(String(format: "%.3f", theta)))"
        case .sGate:
            "S"
        case .tGate:
            "T"
        case let .rotationX(theta):
            "Rx(\(String(format: "%.3f", theta)))"
        case let .rotationY(theta):
            "Ry(\(String(format: "%.3f", theta)))"
        case let .rotationZ(theta):
            "Rz(\(String(format: "%.3f", theta)))"
        case let .cnot(control, target):
            "CNOT(c:\(control), t:\(target))"
        case let .controlledPhase(theta, control, target):
            "CP(\(String(format: "%.3f", theta)), c:\(control), t:\(target))"
        case let .swap(q1, q2):
            "SWAP(\(q1), \(q2))"
        case let .toffoli(c1, c2, target):
            "Toffoli(c1:\(c1), c2:\(c2), t:\(target))"
        }
    }
}
