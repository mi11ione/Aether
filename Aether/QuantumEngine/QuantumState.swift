...
struct QuantumState: CustomStringConvertible {
    ...
    nonisolated var stateSpaceSize: Int {
        1 << numQubits
    }
    ...
    init(numQubits: Int) {
        ...
        // Initialize to |00...0⟩: first amplitude = 1, all others = 0
    ...
    init(numQubits: Int, amplitudes: [Complex<Double>]) {
        ...
        if abs(sumSquared - 1.0) > QuantumConstants.doubleTolerance,
           sumSquared > QuantumConstants.doubleMinThreshold
        {
    ...
    nonisolated func isNormalized() -> Bool {
        ...
        return abs(sum - 1.0) < QuantumConstants.doubleTolerance
    }
    ...
    mutating func normalize() throws {
        ...
        guard sumSquared > QuantumConstants.doubleMinThreshold else {
    ...
    func validate() -> Bool {
        // Check for NaN/Inf
        guard amplitudes.allSatisfy(\.isFinite) else {
            return false
        }
    ...
    var description: String {
        ...
        let threshold = QuantumConstants.displayThreshold
        var terms: [String] = []
...
extension QuantumState: Equatable {
    ...
    nonisolated static func == (lhs: QuantumState, rhs: QuantumState) -> Bool {
        ...
        // Compare amplitudes with tolerance
        for (a, b) in zip(lhs.amplitudes, rhs.amplitudes) {
            if a != b { // Uses Complex<T> equality with tolerance
                return false
            }
        }
